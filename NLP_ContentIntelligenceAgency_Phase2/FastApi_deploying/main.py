...
def upload_feedback_csv_to_azureml(
    ...
    # Use environment variables or placeholders for sensitive credentials
    auth = ServicePrincipalAuthentication(
        tenant_id=os.getenv("AZ_TENANT_ID", "<TENANT_ID>"),
        service_principal_id=os.getenv("AZ_CLIENT_ID", "<CLIENT_ID>"),
        service_principal_password=os.getenv("AZ_CLIENT_SECRET", "<CLIENT_SECRET>")
    )
    ws = Workspace(
        subscription_id=os.getenv("AZ_SUBSCRIPTION_ID", "<SUBSCRIPTION_ID>"),
        resource_group=os.getenv("AZ_RESOURCE_GROUP", "<RESOURCE_GROUP>"),
        workspace_name=os.getenv("AZ_WORKSPACE_NAME", "<WORKSPACE_NAME>"),
        auth=auth
    )
    datastore = Datastore.get(ws, os.getenv("AZ_DATASTORE_NAME", "workspaceblobstore"))
...
def load_model(
    model_path="./fastapi-deploying-model",
...
async def upload_csv(file: UploadFile = File(...)):
    ...
    output_dir = os.getenv("APP_OUTPUT_DIR", "/app/output")
...
async def feedback_csv(
    ...
    local_path = os.path.join(tempfile.gettempdir(), csv_file.filename)
...
async def download_csv():
    path = os.path.join(os.getenv("APP_OUTPUT_DIR", "/app/output"), "csv_predictions.csv")
